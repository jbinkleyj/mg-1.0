...
char *SCCS_Id_perf_hash = "@(#)perf_hash.c	1.4 16 Mar 1994";
...
static void delete(int v, int e) 
...
static void ph_remove(int e, int v)
{
  int b;
  mk[e] = ERASED;
  push(S, e);

  if(NODEa[e] != v) delete(NODEa[e], e);
  if(NODEb[e] != v) delete(NODEb[e], e);
  if(NODEc[e] != v) delete(NODEc[e], e);
...
static int ph_remove(int e, int v)
{
  if (!Stack) 
    {
      StackMax = 256;
...
static int acyclic(int m, int n)
{
...
/* are independent.                                              */
static int tree_builder(int num, uchar **keys, int *n) 
{
...
static void make_trans_func(int num, uchar **keys)
{
  int i, j;
...
{
  if(NODEa) Xfree(NODEa);
  if(NODEb) Xfree(NODEb);
  if(NODEc) Xfree(NODEc);
...
static void all_free(void)
{
  int i;
  temp_free();
  if(translate) Xfree(translate);
  if(g) Xfree(g);
#ifndef STRUCT
  for (i = 0; i<MAX_CH; i++)
    {
      if (tb0 && tb0[i]) Xfree(tb0[i]);
      if (tb1 && tb1[i]) Xfree(tb1[i]);
      if (tb2 && tb2[i]) Xfree(tb2[i]);
    }
  if (tb0) Xfree(tb0);
  if (tb1) Xfree(tb1);
  if (tb2) Xfree(tb2);
...
perf_hash_data *gen_hash_func(int num, uchar **keys, int r)
{
  int n;
  perf_hash_data *phd;
  translate = Xmalloc(sizeof(uchar) * 256);
...
int write_perf_hash_data(FILE *f, perf_hash_data *phd)
{
  ...
  tot += fwrite((char*)phd->translate, sizeof(uchar), 256, f);
...
perf_hash_data *read_perf_hash_data(FILE *f)
{
  ...
  if (tot != 4*sizeof(int)) return(NULL);
  phd->translate = Xmalloc(sizeof(uchar) * 256);
  ...
  if (!(phd->translate && phd->g && phd->tb0 && phd->tb1 && phd->tb2 && ok))
    {
      if(phd->translate) Xfree(phd->translate);
      if(phd->g) Xfree(phd->g);
      for (i = 0; i<MAX_CH; i++)
	{
	  if (phd->tb0 && phd->tb0[i]) Xfree(phd->tb0[i]);
	  if (phd->tb1 && phd->tb1[i]) Xfree(phd->tb1[i]);
	  if (phd->tb2 && phd->tb2[i]) Xfree(phd->tb2[i]);
	}
      if (phd->tb0) Xfree(phd->tb0);
      if (phd->tb1) Xfree(phd->tb1);
      if (phd->tb2) Xfree(phd->tb2);
      Xfree(phd);
      return(NULL);
    }
  tot += fread((char*)phd->translate, sizeof(uchar), 256, f);
  ...
  if (!(phd->translate && phd->g && phd->tb && ok))
    {
      ...
      if(phd->g) Xfree(phd->g);
      for (i = 0; i<MAX_CH; i++)
	if (phd->tb && phd->tb[i]) Xfree(phd->tb[i]);
      if (phd->tb) Xfree(phd->tb);
      Xfree(phd);
      return(NULL);
    }
  tot += fread((char*)phd->translate, sizeof(uchar), 256, f);
...
int perf_hash(perf_hash_data *phd, uchar *s)
{
  int u, v, w, i, l;
  l = *s++;
  i = (l - 1) % phd->MAX_L;
  ...
  while(l) 
    {
#ifndef STRUCT
      if((u += phd->tb0[phd->translate[*s]][i]) >= phd->MAX_N) u -= phd->MAX_N;
      if((v += phd->tb1[phd->translate[*s]][i]) >= phd->MAX_N) v -= phd->MAX_N;
      if((w += phd->tb2[phd->translate[*s]][i]) >= phd->MAX_N) w -= phd->MAX_N;
#else
      struct tb_entry *tbp = &phd->tb[phd->translate[*s]][i];
      if((u += tbp->tb0) >= phd->MAX_N) u -= phd->MAX_N;
      if((v += tbp->tb1) >= phd->MAX_N) v -= phd->MAX_N;
      if((w += tbp->tb2) >= phd->MAX_N) w -= phd->MAX_N;
#endif
      if(++i >= phd->MAX_L) i = 0;
      l--; s++;
    }
  if(u == v && ++v >= phd->MAX_N) v = 0;
  if(w == v) 
    {
      if(++w >= phd->MAX_N) w = 0;
      if(w == u && ++w >= phd->MAX_N) w = 0;
    } 
  else
    if(w == u) 
      {
	if(++w >= phd->MAX_N) w = 0;
	if(w == v && ++w >= phd->MAX_N) w = 0;
      }
