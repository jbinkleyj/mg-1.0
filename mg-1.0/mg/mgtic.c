...
void match_sequence(marklistptr list, marklistptr library, marklistptr *symbol_list,Pixel **recon,int quick)
{
    ...
    for(liststep=list,i=0;liststep; liststep=liststep->next,i++){
      if(V) if(!(i%gap)) fprintf(stderr,".");
      d=liststep->data;
      
      found=-1;score=INT_MAX;
...
*/
	...
	for(r=0;r<d2.h;r++)
	  for(c=0;c<d2.w;c++)if(pbm_getpixel(d2.bitmap,c,r))
	    pbm_putpixel(recon,d.xpos+c-carryx,d.ypos+r-carryy,1);
      }
      else {
...
void main(int argc, char *args[])
{
	...
	for(i=1;i<argc;i++) if(args[i][0]!='-') 
	  if(strcmp(args[i-1],"-R")){  /* ignore arg following -R, ie. -R <filename> */
	    if(!s1) s1=args[i];
	    else if(!s2) s2=args[i];
	    else error(args[0],"too many filenames","");
	  }
	
	if(ticencode==ticdecode) error(args[0],"please specify either encode XOR decode","");
	...
	if(ticencode){
	  ...
	  *****************/
	  long PrevBits=0;
	  long BITS_header=0,BITS_symbols=0,BITS_offsets=0,BITS_residue=0,BITS_footer=0,BITS_library=0;
	  
	  libraryname=s1; if(!libraryname) error(args[0],"please specify a library file","");
	  infile=s2; if(!infile) error(args[0],"please specify a file name","");
	  
	  inf=fopen(infile,"rb");
	  if(inf==NULL) error(args[0],"Trouble opening file:",infile);
	  setbuf(inf,bufferin);
	  
	  count=librarysize=read_library(libraryname,&library);

	  if(V) fprintf(stderr,"%s: processing...\n",args[0]);

	  if(pbm_isapbmfile(inf)){
	    if(V) fprintf(stderr,"reading file %s...\n",infile);
	    bitmap=pbm_readfile(inf,&cols,&rows);
	    fclose(inf);

	    bitmapcopy=pbm_copy(bitmap,cols,rows);
	    
	    if(V) fprintf(stderr,"extracting...");
	    ExtractAllMarks(bitmap,&list,cols,rows);
	    if(V) fprintf(stderr,"(%d marks)\n",marklist_length(list));
	    
	    /* sort into reading order */
	    if(V) fprintf(stderr,"sorting...\n");
	    list=sortmarks(list);
	    ...
	    if(V) fprintf(stderr,"encoding cols, rows, and number of symbols=%d\n",count);
	    ...
	    if(external==0){
	      if(V) fprintf(stderr,"encoding library\n");
	    ...
	    if(V) fprintf(stderr,"encoding symbol sequence\n");
	    InitPPM();
	    ...
	    if(V) fprintf(stderr,"encoding offset sequence\n");
	    ...
	    if(!lossy){
	      if(V) fprintf(stderr,"encoding residue...\n");		

	      d.bitmap=bitmap; d.h=rows; d.w=cols;
	      d2.bitmap=recon;d2.h=rows;d2.w=cols;
		...
		if(temp==NULL) error(args[0],"Trouble creating file:",splitfilename);
		...
		{
		  if(V)	fprintf(stderr,"not encoding residue..lossy mode\n");
		  CloseDownArithEncoding();		  
		  BITS_footer=CountOfBitsOut-PrevBits;
		}
	    ...
	    if(external) fprintf(stderr," (excluding external lib)");
	    fprintf(stderr,", Lossless CR: %4.2f\n",(!lossy)*(cols*rows)/(float)(CountOfBitsOut));
	  }
	  else error(args[0],"unknown format of bitmap--expecting PBM.","");
	}
	else {
	  /*****************
	    DECODING STAGE
	  *****************/
	  ...
	  if(external){
	    libraryname=s1; infile=s2;
	    count=librarysize=read_library(libraryname,&library);
	  }
	  else {
	    infile=s1;
	    if(infile && s2) error(args[0],"too many filenames","");
	  }
	  
	  if(!freopen(infile,"rb",stdin)) error(args[0],"Trouble opening file:",infile);
	  

	  if(V) fprintf(stderr,"decompressing...\n");
	  ...
	  { int version=DecodeGammaDist();
	    if(version!=1) error(args[0],"Need later version of decompressor.","");
	  }
	  
	  { int templossy=DecodeGammaDist();
	    if(!lossy) lossy=templossy;   /* can only choose if encoded file is lossless */
	    if(V){
	      if(lossy) fprintf(stderr,"lossy mode\n");
	      else fprintf(stderr,"lossless mode\n");
	    }
	  }

	  if(DecodeGammaDist()){  /* if compressed file doesn't contain library */
	      if(!external) error(args[0],"compressed file doesn't contain library, specify externally","");
	      external=1;
	    }
	  else { /* if compressed file contains library */
	      if(external) fprintf(stderr,"ignoring external library file\n");
	      external=0;
	    }
	  ...
	  if(external){
	    if(i>librarysize)
	      error(args[0],"external library file is too small!","");
	    else if(i<librarysize)
	      fprintf(stderr,"%s: warning, expecting a different (smaller) library.\n",args[0]);
	  }
	  librarysize=i;
	     
	  if(V) fprintf(stderr,"cols %d, rows %d, num syms %d, library size %d\n",cols,rows,count,librarysize);

	  /* decode library */
	  if(external==0){ 
	    if(V) fprintf(stderr,"reading library\n");
	    bl_clearmodel();
	    bl_readtemplate();
	    for(n=0;n<librarysize;n++){
	      bl_decompress_mark(&d);
	      d.symnum=n;
	      if(library==NULL) step=marklist_add(&library,d);
	      else step=marklist_add(&step,d);
	    }
	    bl_freemodel();
	    if(V) fprintf(stderr,"read %d marks from library\n",marklist_length(library));
	  }
	  ...
	  /* decode symbols */
	  InitPPM();
	  if(V) fprintf(stderr,"decompressing %d symbols\n",count);
	  symbol_list=DecodeSymbols(count);

	  /* decode offsets */
	  if(V)	fprintf(stderr,"reading offsets...\n");
	  ...
	  /* decode the residue */
	  if(!lossy){
	    if(V) fprintf(stderr,"decoding residue...\n");
	    ...
	    if(splitfilename){
	      ...
	      if(temp==NULL) error(args[0],"Trouble opening file:",splitfilename);
	  ...
	  if(V)	fprintf(stderr,"writing pbm file...\n");
	  setbuf(stdout,bufferout);
