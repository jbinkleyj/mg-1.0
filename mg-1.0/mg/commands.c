 ...
 *
 *       @(#)commands.c	1.8 16 Mar 1994
 *
 **************************************************************************/

char *SCCS_Id_commands = "@(#)commands.c	1.8 16 Mar 1994";
...
static void CmdPush(void)
{
  PushEnv();
}


static void CmdPop(void)
{
  PopEnv();
  if (EnvStackHeight() == 1)
    PushEnv();
...
static void CmdConditions(void)
{
  FILE *more;
  int i;
  if (!(more = popen(GetDefEnv("pager","more"),"w")))
...
{
  while (EnvStackHeight() > 1) PopEnv();
  PushEnv();
}

static void CmdDisplay(void)
{
  int i;
  size_t l = 0;
  char *name;
...
static void CmdSet(char *name, char *value)
{
  if (!*name)
    {
  ...
  switch(SetEnv(name, value, NULL))
    {
    case -1 : CommandsErrorStr = "Out of memory\n"; break;
    case -2 : CommandsErrorStr = ConstraintErrorStr; break;
    }
}

static void CmdUnset(char *name)
{
  if (!*name)
    {
      CommandsErrorStr = "Blank names are not permitted";
 ...
 */
static void CmdInput(char *param1, char *param2) 
{
  enum {READ, APPEND, PIPEFROM} mode;
  FILE *NewInput;
  char *buf, *s;
...
static void CmdOutput(char *param1, char *param2) 
{
  enum {WRITE, APPEND, PIPETO} mode;
  char *buf, *s;
  FILE *NewOutput;
...
static char *ParseArg(char *line, char *buffer)
{
  if (*line == '\"')
    {
      line++;
...
char *ProcessCommands(char *line)
{
  struct 
    {
      char *Command;
...
void read_mgrc_file(void)
{
  ...
  if (!mgrc)
    {
      sprintf(FileName,"%s/.mgrc",getenv("HOME"));
      mgrc = fopen(FileName,"r");
    }
