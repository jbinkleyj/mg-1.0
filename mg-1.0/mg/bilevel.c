...
void bl_clearmodel()
{
    ...
    if(!P) error("bl_clearmodel()","not enough memory.","");

    P2=(prob*)malloc(TEMPLATESIZE*sizeof(prob));
    if(!P2) error("bl_clearmodel()","not enough memory.","");
...
void  bl_compress_mark(marktype d)
{
    ...
    for(r=0;r<d.h;r++){
      if(V) if(r&&(r%200==0)) fprintf(stderr,"."); 
...
void  bl_clair_decompress(marktype d1, marktype d2)
{
    ...
    if(V) fprintf(stderr,"cols=%d rows=%d\n",cols,rows);
