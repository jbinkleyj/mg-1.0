...
char * SCCS_Id_text_get = "@(#)text_get.c	1.8 17 Mar 1994";
...
static double FetchDocStartLev1 (text_data *td, u_long DN, 
				 u_long *seek_pos, u_long *len)
{
  unsigned long data[2];
  Fseek (td->TextIdxFile, sizeof(unsigned long) *
	 (DN - 1) + sizeof(unsigned long)*3, 0);
...
static int LoadIdx(text_data *td, unsigned long DN)
{
  if (!td->idx_data)
    {
      td->idx_data = Xmalloc(sizeof(*(td->idx_data))*PAGE_SIZE);
      if (!td->idx_data)
	FatalError(1, "Out of memory in FDSL2");
    }
  if (td->current_pos == -1 || DN >= td->current_pos+PAGE_SIZE-1 ||
      DN < td->current_pos)
    {
      long rn = (long)DN - (PAGE_SIZE >> 1);
      if (rn < 1)
	rn = 1;
      Fseek (td->TextIdxWgtFile, (sizeof(unsigned long) + sizeof(float)) *
	     (rn-1) + sizeof(unsigned long) , 0);
      Fread ((char*)td->idx_data, sizeof(*(td->idx_data))*PAGE_SIZE, 
	     1, td->TextIdxWgtFile);
      td->current_pos = rn;
    }
...
static double FDSL2(text_data *td, unsigned long DN, unsigned long *Pos)
{
...
static double FetchDocStartLev2(text_data *td, u_long DN, 
				u_long *seek_pos, u_long *len)
{
  double Weight;
  unsigned long s1, s2;
...
void FreeTextData(text_data *td)
{
  if (td)
    {
      if (td->idx_data) Xfree(td->idx_data);
      Xfree(td);
    }
}


static int pts_comp(const void *A, const void *B)
{
...
static void FreeAuxDict(auxiliary_dict *ad)
{
  if (!ad) return;
  if (ad->word_data[0])
...
static auxiliary_dict *LoadAuxDict(compression_dict *cd, File *text_aux_dict)
{
  ...
  for (i=0; i <= 1; i++)
	...
	{ mg_errno = MG_NOMEM; FreeAuxDict(ad); return(NULL); }
      if (!(ad->words[i] = Xmalloc(ad->afh[i].num_frags * sizeof(uchar*))))
	{ mg_errno = MG_NOMEM; FreeAuxDict(ad); return(NULL); }

      Fread(ad->word_data[i], ad->afh[i].mem_for_frags, sizeof(uchar), 
	    text_aux_dict);
			    ...
			    comp_frags_header *cfh, uchar **escape)
{
  ...
  if (!(values = Xmalloc((MAX_HUFFCODE_LEN+1) * sizeof(uchar **))))
    return(NULL);
  ...
  if (cfh->hd.clens) Xfree(cfh->hd.clens); cfh->hd.clens = NULL;
  return values;
}


static compression_dict *Load_Comp_Dict(File *dict, File *aux_dict)
{
  int which;
  compression_dict *cd;
...
static compression_dict *Load_Fast_Comp_Dict(File *text_fast_comp_dict)
{
  compression_dict *cd;
  u_long *p, *end;
  uchar *fixup;
  ...
  Fread(cd, sizeof(uchar), mem, text_fast_comp_dict);

  if (!(fixup = Xmalloc(fixup_mem)))
    { mg_errno = MG_NOMEM; return (NULL); }

  Fread(fixup, fixup_mem, sizeof(uchar), text_fast_comp_dict);
...
int DecodeText(compression_dict *cd,
	       uchar *s_in, int l_in, uchar *s_out, int *l_out)
{
