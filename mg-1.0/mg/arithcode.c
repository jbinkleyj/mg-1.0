...
char *SCCS_id_arithcode = "@(#)arithcode.c	1.8 16 Mar 1994";
...
void EncodeGammaSigned(int snum, int *pos, int *neg)
{
  ...
  if(*pos+(*neg)>=maxfrequency){ *pos=(*pos+1)/2; *neg=(*neg+1)/2;}
...
int DecodeGammaSigned(int *pos, int *neg)
{
  ...
  if(signedpos) return DecodeGammaDist();
  else return (-DecodeGammaDist());
}
