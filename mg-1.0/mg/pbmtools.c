...
Pixel **pbm_allocarray(int cols, int rows)
{
	...
	if(p==NULL) error("pbm_allocarray","out of memory","");
	for(i=0;i<rows;i++){
		p[i]=(Pixel*)calloc(newc,sizeof(Pixel));
		if(p[i]==NULL) error("pbm_allocarray","out of memory","");
		}
	return p;
}
...
void pbm_writenamedfile(char fn[], Pixel **bitmap, int cols, int rows)
{
    ...
    tempfp=fopen(fn,"wb");
    if(tempfp==NULL) error("pbm_writenamedfile","can't open file:",fn);
