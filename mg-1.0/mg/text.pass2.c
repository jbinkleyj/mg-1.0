...
char *SCCS_Id_text_pass2 = "@(#)text.pass2.c	1.7 16 Mar 1994";
...
int init_text_2(char *file_name)
{
  ...
  if (!(comp_buffer = Xmalloc(sizeof(uchar) * buf_size)))
    {
...
#if 0
  MaxMemInUse += sizeof(uchar) * buf_size;
#endif
...
int process_text_2(uchar *s_in, int l_in)
{
...
int write_aux_dict(char *FileName)
{
  int i;
  FILE *aux;
  ...
  for (i=0; i<=1; i++)
	...
	afh.mem_for_frags += POOL_SIZE - cp->left;
      ...
      for (cp = nht[i].first_pool; cp; cp = cp->next)
	fwrite(cp->pool, POOL_SIZE - cp->left, sizeof(uchar), aux);
    }
  fclose(aux);
  return COMPALLOK;
}
...
void estimate_compressed_aux_dict(void)
{
  int i;
