 ...
 *
 *       @(#)mg_compression_dict.c	1.6 16 Mar 1994
 *
 **************************************************************************/
...
char *SCCS_Id_comp_dict_process = "16 Mar 1994 @(#)mg_compression_dict.c	1.6";
...
void main(int argc, char **argv)
{
  ...
  while ((ch = getopt (argc, argv, "0123CPSf:d:l:hk:HBDYM")) != -1)
	...
	break;
      case 'h' :
      case '?' :
	printf ("usage: %s [-l lookback] [-f input_file]"
...
static void ReadInWords(char *filename)
{
  FILE *f;
  unsigned long i;
...
static void Alloc_keep_discard(void)
{
  keep[0].num_wds = 0;
...
static void SortAndCount_DictData(DictData *dd)
{
  int i;
  WordData **wd;
...
static int BigSaving(void *a, void *b)
{
  WordData *aa = *(WordData **)a;
  WordData *bb = *(WordData **)b;
...
static int SmallSaving(void *a, void *b)
{
  WordData *aa = *(WordData **)a;
  WordData *bb = *(WordData **)b;
  return (aa->saving < bb->saving) ? -1 : (aa->saving > bb->saving);
...
static void CalcCharCounts(WordData **wd, int num,
...
void CalcBitCost(WordData **discard_word, int discard_num,
		 ...
		 u_long escape[2], int num_trans)
{
  ...
  Xfree(char_lens[0]); Xfree(char_lens[1]);
  Xfree(len_lens[0]); Xfree(len_lens[1]);
}
...
static void Method3(int k)
{
  ...
  discard_heap = malloc(discard_num * sizeof(WordData *));

  keep_num = 0;
  keep_heap = malloc(discard_num * sizeof(WordData *));
  ...
  while (keep_num && discard_num)
	...
	if (keep_heap[0]->saving < discard_heap[0]->saving)
	  {
...
static void Write_cdh(FILE *f, compression_dict_header *cdh)
{
  fwrite(cdh, sizeof(*cdh), 1, f);
}


static void Write_words(FILE *f, DictData *dd)
{
  int i;
  uchar *curr, *prev = NULL;
  for (i = 0; i < dd->num_wds; i++) 
	...
	len = 0;
      ...
      fwrite(curr+len+1, sizeof(uchar), curr[0]-len, f);
      prev = curr;
    }
...
static int Uncompressed_size(DictData *dd)
{
  int i, us;
...
static u_long Write_data(FILE *f, DictData *dd, int lookback)
{
  u_long mem_reqd;
  huff_data *hd;
  int i, us = dd->num_wds;
  ...
  uchar *lastword[MAX_HUFFCODE_LEN+1];
...
static void Write_charfreqs(FILE *f, DictData *dd, int words, 
			      Bool zero_freq_permitted)
{
  int j;
  long freqs[256];
  WordData **wd = dd->wd;
...
static void Write_wordlens(FILE *f, DictData *dd, Bool zero_freq_permitted)
{
  int j;
  long freqs[16];
  WordData **wd = dd->wd;
...
static u_long WriteOutWords(char *file_name, u_long type, int lookback)
{
  FILE *f;
  int i;
  u_long mem_reqd = 0;
  ...
  bzero((char*)&cdh.params, sizeof(cdh.params));
  ...
  for (i=0; i<2; i++)
      ...
      case MG_PARTIAL_DICTIONARY :
	{
	  ...
	  Write_charfreqs(f, &discard[i], i, TRUE);
	  Write_wordlens(f, &discard[i], TRUE);
	}
	break;
      case MG_SEED_DICTIONARY :
	{
	  ...
	  switch (novel_method)
	    ...
	    case MG_NOVEL_HUFFMAN_CHARS :
	      Write_charfreqs(f, &all[i], i, FALSE);
	      Write_wordlens(f, &all[i], FALSE);
	      break;
