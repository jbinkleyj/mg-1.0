 ...
 *
 *       @(#)query.ranked.c	1.9 16 Mar 1994
 *
 **************************************************************************/
...
char *SCCS_Id_query_ranked = "@(#)query.ranked.c	1.9 16 Mar 1994";
...
Heap *Heap_Make(int size, HeapComp hc)
{
  Heap *H;
...
void Heap_DeleteHead(Heap *H)
{
  H->HE[0] = H->HE[--H->NumItems];
  Heap_Heapify(H, 1);
}

int Heap_Lesser(HeapEntry *a, HeapEntry *b)
{
...
int Heap_Greater(HeapEntry *a, HeapEntry *b)
{
  return (a->Weight > b->Weight ? 1 : 
	  (a->Weight == b->Weight ? 0 : -1));
}

int Make_Exact_Root(query_data *qd, Heap *H)
{
  int num = 0;
  HeapEntry *he = H->HE;
  while (he->SeekPos == 0)
...
static TermList *MakeTermList(int n)
{
  TermList *t;
  if (!(t = Xmalloc(sizeof(TermList) + (n-1)*sizeof(TermEntry))))
    return(NULL);
  t->num = 0;
  return t;
}
...
static TermList *ResizeTermList(TermList *t, int n)
{
  if (!(t = Xrealloc(t, sizeof(TermList) + (n-1)*sizeof(TermEntry))))
    return(NULL);
  return t;
}
...
int doc_count_comp(const void *A, const void *B)
{
  const TermEntry *a = A;
  const TermEntry *b = B;
...
static TermList *ParseRankedQuery(stemmed_dict *sd, char *QueryLine,
				  Bool Sort)
{
  uchar	Word[MAXSTEMLEN + 1];
  uchar	chbuf, *end, *s_in;
  TermList *Terms = MakeTermList(0);
  s_in = (uchar*)QueryLine;
  end = s_in + strlen((char*)s_in);
  ...
  while ( s_in <= end) 
	...
	if (compare(Terms->TE[j].Word, Word) == 0) break;
      ...
      else
	{
	  ...
	  if ((word_num = FindWord(sd, Word, &count, &doc_count, 
				   &invf_ptr, &invf_len)) != -1)
	    {
	      ...
	      for (j = 0; j < Terms->num; j++)
		if (Terms->TE[j].WE.word_num == word_num) break;
		...
		{
		  Terms = ResizeTermList(Terms, Terms->num+1);
		  Terms->TE[Terms->num].WE.word_num = word_num;
		  Terms->TE[Terms->num].WE.count = count;
		  Terms->TE[Terms->num].WE.doc_count = doc_count;
		  Terms->TE[Terms->num].WE.invf_ptr = invf_ptr;
		  Terms->TE[Terms->num].WE.invf_len = invf_len;
		  Terms->TE[Terms->num].Count = 1;
		  Terms->TE[Terms->num].Word = Xmalloc(Word[0]+1);
		  bcopy((char*)Word, (char*)(Terms->TE[Terms->num].Word), 
			Word[0]+1);
		  Terms->num++;
		}
	    }
 ...
 */
static DocList *CosineGet(query_data *qd, TermList *Terms, 
			  RankedQueryInfo *rqi)
{
  DocList *Docs;
  ...
  switch (rqi->AccumMethod)
    ...
    case 'A' : 
      ...
      if (!AccumulatedWeights) return NULL;
      break;
    case 'H' : 
  ...
  switch (rqi->AccumMethod)
	...
	for (d = AccumulatedWeights; i; i--, d++)
      ...
      break;
    case 'H' : 
      MaxParas = HT->num + HT->Suplimentary_Num; break;
    case 'L' : 
      MaxParas = LT->num; break;
    }
	...
	for (;ideh; ideh = ideh->next)
	  {
	    ...
	    if (ideh->IDE.Sum <= he->Weight) continue;
	    *he->OrgWeight = he->Weight;
	    he->DocNum = ideh->IDE.DocNum+1;
...
void RankedQuery(query_data *qd, char *Query, RankedQueryInfo *rqi)
{
  ...
  TermList *tl;
  DocList *dl;
  tl = ParseRankedQuery(qd->sd, Query, rqi->Sort);
  dl = CosineGet(qd, tl, rqi);
  if (!dl)
