...
void Encode_bitmap(marktype b, char template[])
{
  InitArithEncoding();
...
void main(int argc, char *args[])
{
	...
	for(i=1;i<argc;i++){
	    if(strcmp(args[i],"-e")==0) encode=1;
	    else if(strcmp(args[i],"-d")==0) decode=1;
	    else if(strcmp(args[i],"-h")==0) usage();
	    ...
	    else error(args[0],"only specify 1 filename","");
	}
	if(encode==decode){
	    fprintf(stderr,"You must specify either encode XOR decode.\n");exit(1);
	}

	if(!filename){fprintf(stderr,"You must specify a filename.\n");exit(1);}

	if(encode){
	    bitmap=pbm_readnamedfile(filename,&d.w,&d.h);
	    if(bitmap==NULL) error(args[0],"problem reading bitmap","");
	...
	else if(decode)
	{
	    ...
	    if(!tpin)error("mgbilevel","Can't open file:",filename);
	    arith_in=tpin;
