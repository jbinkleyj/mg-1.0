...
int maxst=0,st=0; struct {int x,y;} a[MAX_FILL_STACK_SIZE];

#define push(i,j) a[st].x=i;a[st].y=j;st++;if(st>MAX_FILL_STACK_SIZE){error("extractor/flood_fill","woooo! Fill stack overflow!","");}
...
void RunRegionFill(Pixel **bitmap,marktype *mark,int i,int j,int xl,int yt,int cols,int rows)
{
	...
	while(st>0){
		...
		while(pbm_getpixel(bitmap,l-1,j)) l--;
		while(pbm_getpixel(bitmap,r+1,j)) r++;
		start_up=start_down=1;
		for(t=min(r+1,cols-1);t>=max(l-1,0);t--) {
			if((t>=l)&&(t<=r)){
				pbm_putpixel(bitmap,t,j,0);

				if(mark) pbm_putpixel(mark->bitmap,t-xl,j-yt,1);
				}

			if((j-1>=0)&&(pbm_getpixel(bitmap,t,j-1))){
				if(start_up) {push(t,j-1);start_up=0;}
				}
			else start_up=1;
			if((j+1<rows)&&(pbm_getpixel(bitmap,t,j+1))){
				if(start_down) {push(t,j+1);start_down=0;}
				}
			else start_down=1;
			}
		}
...
int ExtractNextMark(Pixel **bitmap,marklistptr *marklist,int *tx, int *ty,int cols, int rows)
{
	...
	i=*tx;j=*ty;
	if((i<0)||(j<0)) error("ExtractNextMark()","Stupid values for x,y in findnextmark()","");
	while(!pbm_getpixel(bitmap,i,j)){
		i++;if(i>=cols){i=0;j++; if(j>=rows)
			return 0;}
		}
	...
	*tx=i;*ty=j;
	...
	do{
		...
		if(j>yb) yb=j-1;
		tempdir=(dir+1)%4;
		if(gp(i+boundary_xcomp(tempdir),j+boundary_ycomp(tempdir))==0) dir=tempdir;
		else {tempdir=dir;
			if(gp(i+boundary_xcomp(tempdir),j+boundary_ycomp(tempdir))==0) dir=tempdir;
			else{
				tempdir=(dir+3)%4;
...
static void normalise(float *p, int w)
{
  int x;
  float mx;
...
static int possibly_joined(marktype *d)
{
  float aspect;
  int i,n,x,y;
  int leftable,l2;
  ...
  if((aspect>=1.1) || (d->w < MAXPROFILE)) { 
    ...
    for(x=0;x<d->w;x++){
      profile[x]=0;
      for(y=0;y<d->h;y++)
	if(pbm_getpixel(d->bitmap,x,y)) profile[x]++;
    }
	...
	if((fabs( (abs(x-leftable)/(float)d->h) - 0.7) >0.3) &&
	   (fabs( (abs(x-l2)/(float)d->h) - 0.7) >0.3)) p2[x]=0;
	if(p2[x]==1) {
	  leftable=l2;
	  l2=x;
	}
    ...
    while(x<=y){
      if(p2[x]){
	if(abs(x-l2)<max(d->h*0.6,5)) p2[x]=0;
	if(p2[x]) l2=x;
      }
      if(p2[y]){
	if(abs(y-leftable)<max(d->h*0.6,5)) p2[y]=0;
	if(p2[y]) leftable=y;
      }
      x++;
      y--;
...
static void split_mark(marklistptr *list, marktype *joinedmark)
{
  marktype new,old;
  ...
  do{
    ...
    for(x=left;x<old.w;x++)
      if(p2[x]) {found=x;break;}
    if(left==old.w-1) break; /* if we've got to the right */
    if(found==0) found=old.w-1;
  } while(found); 
...
void ExtractAllMarks(Pixel **bitmap, marklistptr *list, int cols, int rows)
{
