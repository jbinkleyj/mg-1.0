 ...
 *
 *       @(#)ivf.pass2.c	1.9 16 Mar 1994
 *
 **************************************************************************/

char *SCCS_Id_ivf_pass2 = "@(#)ivf.pass2.c	1.9 16 Mar 1994";
...
static int open_files(char *file_name)
{
  ...
  sprintf(FName,"%s/%s.invf.state.%d", get_basepath(), file_name, getpid());
  ...
  sprintf(FName,"%s/%s.chunk.state.%d", get_basepath(), file_name, getpid());
  ...
  sprintf(FName,"%s/%s.chunks.%d", get_basepath(), file_name, getpid());
...
invf_state_rec *in_cache(int pos)
{
  ...
  if (isr_pos >= 0)
#ifdef SUNOS5
    memmove(&isr_data[isr_pos * ISR_ENTRY_SIZE], &isr, ISR_ENTRY_SIZE);  
#else
...
#ifdef SUNOS5
  memmove(&isr, &isr_data[isr_pos * ISR_ENTRY_SIZE], ISR_ENTRY_SIZE);  
#else
...
unsigned long occur_to_lexical(long occ)
{
  static long pos = -1;
...
void add_chunk_state(unsigned long pos, unsigned long start_doc, 
		     unsigned long N)
{
...
int init_ivf_2(char *file_name)
{
  ...
  totalHbytes = sizeof(perf_hash_data) + sizeof(uchar)*256 + 
  ...
  if (!(lg_bs = Xmalloc(sizeof(uchar) * idh.dict_size)))
    {
      Message("No memory for lg b's");
      return COMPERROR;
    }
  totalDbytes += sizeof(uchar) * idh.dict_size;
...
#ifdef SUNOS5
  memset(counts, 0, sizeof(unsigned long) * dict_size);
#else
...
#ifdef SUNOS5
  memset(chunk_ptrs, 0, chunks_read * sizeof(unsigned long));
#else
...
#ifdef SUNOS5
      memmove(rbsi[i].Buf, crbs.Buf, rbsi[i].len);
#else
...
#ifdef SUNOS5
	  memmove(chks[j].Buf, chks[0].Buf, chks[0].len);
#else
  ...
  for (i=0; i<dict_size; i++)
      ...
      for (j=0; j<chunks_read; j++)
	{
	  ...
	  if (p)
		...
		{
		  int k, CurrDoc;
		  DECODE_START((uchar*)MemoryBuffer, ptr)
...
static int wl_comp(const void *a, const void *b)
{
  return *((int*)a) - *((int*)b);
}
...
static int process_doc(uchar *s_in, int l_in)
{
  ...
  while ( s_in <= end) 
	...
	ENCODE_START((uchar*)MemoryBuffer, arr->ptr)
...
int process_ivf_2(uchar *s_in, int l_in)
{
  if (InvfLevel <= 2)
...
static void stats(unsigned long len)
{
#ifndef SILENT
  fseek(count, 0, 2);
  ...
  for (i=0; i<dict_size; i++) 
    {
      ...
      while (s < e)
	{
	  uchar c = getc(invf_in);
	  if (s == e-1)
	    {
	      uchar ands[8] = {0xff, 0x80, 0xc0, 0xe0, 0xf0, 0xf8, 0xfc, 0xfe};
