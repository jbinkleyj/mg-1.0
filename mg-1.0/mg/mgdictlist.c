...
char *SCCS_Id_dictlist = "@(#)mgdictlist.c	1.6 16 Mar 1994";
  ...
  for (i=0; i<idh.dict_size; i++)
    {
      ...
      suff = getc(f);
      *prev = copy+suff;
      fread(prev+copy+1, sizeof(uchar), suff, f);
  ...
  for (i = 0; i < cfh.hd.num_codes; i++)
    {
      ...
      fread(prev+copy+1, sizeof(uchar), val, f);
      *prev = val+copy;
...
void ReadCharHuffman(FILE *f, char *title)
{
  int i;
  huff_data hd;
  u_long *codes;
...
void ReadLenHuffman(FILE *f, char *title)
{
  int i;
  huff_data hd;
  u_long *codes;
