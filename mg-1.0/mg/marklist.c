...
void marktype_calc_centroid(marktype *b)
{
  ...
  for(r=0;r<b->h;r++)
    for(c=0;c<b->w;c++)
      if(pbm_getpixel(b->bitmap,c,r)) {count++;xtot+=c;ytot+=r;}
  if(count){
    xtot/=count;
    ytot/=count;
  }
...
static void marktype_area(marktype *d)
{
  int x,y;
  int set=0;
  ...
  for(y=0;y<d->h;y++)
    for(x=0;x<d->w;x++)
      if(pbm_getpixel(d->bitmap,x,y)) set++;
  d->set=set;
}


static void marktype_h_runs(marktype *d)
{
    int x,y;
    int inrun=0,num=0;
    int foundblack=0;
...
static void marktype_v_runs(marktype *d)
{
    int x,y;
    int inrun=0,num=0;
    int foundblack=0;
...
static void marktype_prog_cent(marktype *d)
{
    int x,y;
    int xtot=0,ytot=0;
    int num=0;
    ...
    for(x=0;x<d->xcen;x++)
	for(y=0;y<d->ycen;y++)
	    if(pbm_getpixel(d->bitmap,x,y)) {xtot+=x;ytot+=y;num++;}
    if(num==0) {
	d->xcen1=-1;d->ycen1=-1;
    }
    else {    
...
{
  if(strlen(str)) strcpy(d->name,str);
...
void marktype_adj_bound(marktype *m)
{
  ...
  for(y=0;y<h;y++)
    for(x=0;x<w;x++)
      if(pbm_getpixel(m->bitmap,x,y)) {top=y; x=w;y=h;}

  for(y=h-1;y>=0;y--)
    for(x=0;x<w;x++)
      if(pbm_getpixel(m->bitmap,x,y)) {bot=y; x=w; y=-1;}

  for(x=0;x<w;x++)
    for(y=0;y<h;y++)
      if(pbm_getpixel(m->bitmap,x,y)) {left=x; x=w; y=h;}

  for(x=w-1;x>=0;x--)
    for(y=0;y<h;y++)
      if(pbm_getpixel(m->bitmap,x,y)) {right=x; x=-1; y=h;}
...
void marklist_average(marklistptr list, marktype *avgmark)
{
    ...
    w=w + (r-l);
    h=h + (b-t);
    if((w>=100) || (h>=100)) matched=0;
    
    if(matched>1){  
      int x_off,y_off;
      for(x=0;x<w;x++)
	for(y=0;y<h;y++) a[x][y]=0;
...
marklistptr marklist_add(marklistptr *list, marktype m)
{
	...
	if(*list==NULL){
		*list=(marklistptr)malloc(sizeof(marklisttype));
		if((*list)==NULL) error("marklist_add","OUT of Memory in marklist_add","");
		p=*list;
		}
	else{
		p=*list;
		...
		if(p->next==NULL) error("marklist_add","OUT of Memory in marklist_add","");
		p=p->next;
		}
	p->data=m;
	p->next=NULL;
...
marklistptr marklist_getat(marklistptr list, int pos, marktype *d)
{
	...
	while(list){
		if(count==pos) {*d=list->data;return list;}
		count++;
		list=list->next;
		}
...
int read_library_mark(FILE *fp, marktype *d)
{
  ...
  if((numread!=2)||(strcmp(str1,"Mark:")!=0))  return 0;
  fscanf(fp,"%s %s\n",str1,(d->name));
  fscanf(fp,"%s %d %s %d\n",str1,&(d->w),str2,&(d->h));
  cols=d->w;
  rows=d->h;
...
int read_library(char libraryname[], marklistptr *library)
{
  ...
  if(lib==NULL) error("read_library()","Trouble opening library file","");
  *library=NULL;
  count=0;
  while(!isEOF(lib)){
    err=read_library_mark(lib,&d);
    if(!err) error("read_library()","unknown format of the library file.","");
    d.symnum=count;
    count++;
...
void elem_add(elemlist *list, int num)
{
  ...
  if(list->head==NULL){
    ...
    if(temp->num==num) 
      ...
      if(nn==NULL) error("","ekk","");
      
      if(num<temp->num){
	nn->num=temp->num;nn->pos=temp->pos;nn->freq=temp->freq;
	nn->next=temp->next;
	temp->num=num;temp->pos=p+f;temp->freq=1;
	temp->next=nn;
      }
      else {
	nn->next=temp->next;temp->next=nn;
	nn->pos=temp->pos+temp->freq;nn->num=num;
	nn->freq=1;
	temp=nn;
      }
    } 
