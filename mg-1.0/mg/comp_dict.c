...
char *SCCS_Id_comp_dict = "@(#)comp_dict.c	1.6 16 Mar 1994";
...
static dict_hash_table *ReadInWords(FILE *dict, comp_frags_header *cfh, 
			       int esc)
{
  int i;
  uchar *allwords, *prev = NULL;
  dict_hash_table *ht;
  uchar **words;
  u_long ht_size; 
  ...
  if (!(ht->words = Xmalloc(sizeof(uchar*) * cfh->hd.num_codes)))
    {
...
#ifdef SUNOS5
  memset(ht->table, 0, ht_size * sizeof(ht->table[0]));
#else
  bzero((char*)ht->table, ht_size * sizeof(ht->table[0]));
#endif

  if (!(allwords = Xmalloc(sizeof(uchar) * cfh->uncompressed_size)))
    {
  ...
  for (i = 0; i < cfh->hd.num_codes; i++, words++) 
    {
      ...
      *words = allwords;
      ...
      fread(allwords+copy+1, sizeof(uchar), val, dict);
      *allwords = val+copy;
