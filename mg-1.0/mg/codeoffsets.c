...
void EncodeOffsets(marklistptr symbol_list, int nsyms)
{
  ...
  if(!xcond) error("EncodeOffsets","Out of memory...","");
  ycond=(elemlist*)malloc(nsyms*sizeof(elemlist));
  if(!ycond) error("EncodeOffsets","Out of memory...","");

  for(t=0;t<nsyms;t++){
    xcond[t].head=ycond[t].head=NULL;
    xcond[t].tot=ycond[t].tot=0;
  }
  xlist.head=NULL; xlist.tot=0;
  ylist.head=NULL; ylist.tot=0;
  for(step=symbol_list,count=0; step; step=step->next,count++){ 
    ...
    for(dox=1;dox>=0;dox--){
      ...
      if(dox) found=elem_find(&xcond[symnum],xoff,&p,&f,&t);
      else found=elem_find(&ycond[symnum],yoff,&p,&f,&t);
	...
	if(dox) found=elem_find(&xlist,xoff,&p,&f,&t);
	else found=elem_find(&ylist,yoff,&p,&f,&t);
	...
	if(found){
	  ...
	  levelbot+=BOT_INC;
	  if(dox) EncodeGammaSigned(xoff,&xpos,&xneg);
	  else EncodeGammaSigned(yoff,&ypos,&yneg);
	}
      }
      if(dox) {
	elem_add(&xlist,xoff);
...
void DecodeOffsets(marklistptr symbol_list, int nsyms)
{
  int xpos=1,xneg=1,ypos=1,yneg=1;
  marklistptr step;
  int i,target,dox;
  ...
  for(step=symbol_list,count=0; step; step=step->next,count++){   
	...
	if(dox)i=arithmetic_decode_target(xcond[symnum].tot);
	else i=arithmetic_decode_target(ycond[symnum].tot);
	...
	if(dox)i=arithmetic_decode_target(xlist.tot);
	else i=arithmetic_decode_target(ylist.tot);
	...
	levelbot+=BOT_INC;
	if(dox) xoff=DecodeGammaSigned(&xpos,&xneg);
	else yoff=DecodeGammaSigned(&ypos,&yneg);
      }
