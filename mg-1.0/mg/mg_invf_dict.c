 ...
 *
 *       @(#)mg_invf_dict.c	1.7 16 Mar 1994
 *
 **************************************************************************/
...
char *SCCS_Id_stem_process = "@(#)mg_invf_dict.c	1.7 16 Mar 1994";
...
static void process_files(char *filename)
{
  ...
  for (i=0; i<idh.dict_size; i++)
    {
      ...
      suff = getc(id);
      *prev = copy+suff;
      fread(prev+copy+1, sizeof(uchar), suff, id);
	  ...
	  sizeof(First_word) + sizeof(invf_ptr) + sizeof(invf_len)> block_size)
	{
	  ...
	  fwrite(buffer, sizeof(uchar), buf_in_use, tmp);
	  ...
	  if (force && chunk < block_size)
	    {
	      fwrite(buffer, sizeof(uchar), block_size-chunk, tmp);
	      chunk = block_size;
	    }
	...
	FatalError(1,"Fatal Internal Error # 34876234\n");
      if (word_num == 0)
	{
	  fwrite(prev, sizeof(uchar), *prev+1, idb);
  ...
  if (force)
	...
	{
	  bzero((char*)buffer, block_size);
	  fwrite(buffer, sizeof(uchar), block_size-amount, idb);
	  sdh.blocks_start += block_size-amount;
	}
    }
	
  while ((num = fread(buffer, sizeof(uchar), block_size, tmp)) != 0)
    fwrite(buffer, sizeof(uchar), num, idb);
  fclose(tmp);
