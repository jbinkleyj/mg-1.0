...
static File *OpenFile(char *base, char *suffix, unsigned long magic, int *ok)
{
  char FileName[512];
  File *F;
...
static int open_all_files(query_data *qd)
{
  int ok = 1;
...
static void close_all_files(query_data *qd)
{
  Fclose(qd->File_text);
  if (qd->File_fast_comp_dict)
    Fclose(qd->File_fast_comp_dict);
...
query_data *InitQuerySystem(char *dir, char *name, InitQueryTimes *iqt)
{
  ...
  if (s && *(s+1) == '\0')
	...
	{ mg_errno = MG_NOMEM; Xfree(qd); return(NULL); }
  ...
  if (open_all_files(qd) == -1)
    { Xfree(qd->pathname); Xfree(qd); return(NULL); }
  ...
  if (!(qd->sd = ReadStemDictBlk(qd->File_stem)))
    { close_all_files(qd); Xfree(qd->pathname); Xfree(qd); return(NULL); }
...
#ifdef TREC_MODE
  {
    ...
    char FileName[512];
    FILE *f;
    if (!strstr(qd->pathname, "trec")) goto error;
...
uchar *GetDocText(query_data *qd, unsigned long *len)
{
  ...
  DecodeText(qd->cd, (uchar*)(DE->CompTextBuffer), DE->Len, 
	     (uchar*)(qd->TextBuffer), &ULen);
