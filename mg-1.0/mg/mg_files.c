		...
		u_long magic_num, int err_mode)
{
  ...
  if (!f)
    { err = "Unable to open \"%s\""; goto error; }

  if (magic_num)
    {
      if (fread(&magic, sizeof(magic), 1, f) != 1)
	{ err = "No magic number \"%s\""; goto error; }
      
      if (!IS_MAGIC(magic))
	{ err = "No MG magic number \"%s\""; goto error; }
      
      if (magic != magic_num)
	{ err = "Wrong MG magic number \"%s\""; goto error; }
    }
  return f;
		  ...
		  u_long magic_num, int err_mode)
{
  ...
  if (magic_num)
    if (fwrite(&magic_num, sizeof(magic_num), 1, f) != 1)
      { err = "Couldn't write magic number \"%s\""; goto error; }
