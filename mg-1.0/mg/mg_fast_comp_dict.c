 ...
 *
 *       @(#)mg_fast_comp_dict.c	1.2 16 Mar 1994
 *
 **************************************************************************/


char *SCCS_Id_mg_make_fast_dict = "@(#)mg_fast_comp_dict.c	1.2 16 Mar 1994";
...
#define WORDNO(p, base) ((((char*)(p))-((char*)(base)))/sizeof(uchar*))
...
void main(int argc, char **argv)
{
  ...
  while ((ch = getopt (argc, argv, "f:d:h")) != -1)
    switch (ch) 
      ...
      case 'h' :
      case '?' :
        printf ("usage: %s [-f input_file] [-d data directory] [-h]\n",
                argv[0]);
        exit(1);
      }
  GetTime(&StartTime);
  ...
  fixup_mem = (ALIGN_SIZE(mem, uchar *) / sizeof(uchar *) + 7) / 8;

  cur = buffer = Xmalloc(mem); bzero(buffer, mem);
  fixup  = Xmalloc(fixup_mem); bzero(fixup, fixup_mem);
...
static u_long mem_for_aux_dict(compression_dict_header *cdh, char *filename)
{
  int i;
  ...
  for (i=0; i <= 1; i++)
    {
      ...
      mem += afh.num_frags * sizeof(uchar*);
      mem = ALIGN_SIZE(mem + afh.mem_for_frags, uchar*);
...
static u_long mem_for_words(FILE *dict, compression_dict_header *cdh,
		     comp_frags_header *cfh)
{
  ...
  mem += ptrs_reqd * sizeof(uchar *);
  mem += (MAX_HUFFCODE_LEN+1) * sizeof(uchar **);
  mem += mem_reqd;
...
void *getmem(u_long size, int align)
{
  void *res;
...
static auxiliary_dict *LoadAuxDict(compression_dict_header *cdh,
				   char *filename)
{
  auxiliary_dict *ad;
  int i;
  ...
  ad = getmem(sizeof(auxiliary_dict), sizeof(uchar*));
  ...
  for (i=0; i <= 1; i++)
    {
      ...
      ad->words[i] = getmem(ad->afh[i].num_frags * sizeof(uchar*), 
			    sizeof(uchar*));
      FIXUP(&ad->words[i]);

      fread(ad->word_data[i], ad->afh[i].mem_for_frags, sizeof(uchar), aux); 
...
static uchar ***ReadInWords(FILE *dict, compression_dict *cd, 
			    comp_frags_header *cfh, uchar **escape)
{
  int i, lookback;
  int ptrs_reqd = 0;
  int mem_reqd = 0;
  int num_set[MAX_HUFFCODE_LEN+1];
  uchar *next_word[MAX_HUFFCODE_LEN+1];
  uchar **vals;
  uchar ***values;
  uchar word[MAXWORDLEN+1];
  uchar last_word[MAX_HUFFCODE_LEN+1][MAXWORDLEN+1];
  ...
  vals = getmem(ptrs_reqd * sizeof(*vals), sizeof(uchar *));

  values = getmem((MAX_HUFFCODE_LEN+1) * sizeof(uchar **), sizeof(uchar **));

  next_word[0] = getmem(mem_reqd, sizeof(char));

  cd->MemForCompDict += ptrs_reqd * sizeof(*vals) + 
    (MAX_HUFFCODE_LEN+1) * sizeof(uchar **) +
      mem_reqd;
  ...
  for (i = 0; i < cfh->hd.num_codes; i++) 
    {
      ...
      fread(word+copy+1, sizeof(uchar), val, dict);
      *word = val+copy;
...
static unsigned long **Generate_Fast_Huffman_Vals(huff_data *data, u_long *mem)
{
  int i;
...
static void load_comp_dict(char *filename)
{
  ...
  cd = getmem(sizeof(compression_dict), sizeof(uchar*));
  cd->fast_loaded = TRUE;
  ...
  for (which = 0; which < 2; which++)
    switch (cd->cdh.dict_type)
      ...
      case MG_COMPLETE_DICTIONARY :
	{
	  cd->cfh[which] = getmem(sizeof(*cd->cfh[which]), sizeof(uchar*));
      ...
      case MG_PARTIAL_DICTIONARY :
	{
	  ...
	  cd->chars_huff[which] = hd; FIXUP(&cd->chars_huff[which]);
	  cd->chars_vals[which] = vals; FIXUP(&cd->chars_vals[which]);
	  if (hd->clens) Xfree(hd->clens); hd->clens = NULL;

	  hd = getmem(sizeof(huff_data), sizeof(char*));
	  cd->MemForCompDict += sizeof(huff_data);
	  Read_Huffman_Data(dict, hd, &cd->MemForCompDict, NULL);
	  vals = Generate_Fast_Huffman_Vals(hd, &cd->MemForCompDict);
	  cd->lens_huff[which] = hd; FIXUP(&cd->lens_huff[which]);
	  cd->lens_vals[which] = vals; FIXUP(&cd->lens_vals[which]);
	  if (hd->clens) Xfree(hd->clens); hd->clens = NULL;
	}
	break;
      case MG_SEED_DICTIONARY :
...
static void save_fast_dict(char *filename)
{
  ...
  fwrite(buffer, sizeof(uchar), mem, fdict);
  fwrite(fixup, sizeof(uchar), fixup_mem, fdict);
