...
char *SCCS_Id_lists = "@(#)lists.c	1.6 16 Mar 1994";
...
DocList *IntersectLists (DocList *List1, DocList *List2)
{
  ...
  if (!List2 || !List1)
    {
      if (List1) Xfree(List1);
      if (List2) Xfree(List2);
  ...
  while (s1-List1->DE < List1->num && s2-List2->DE < List2->num)
	...
	while (s2-List2->DE < List2->num && s2->DocNum < s1->DocNum) s2++;
      if (s2-List2->DE < List2->num)
	while (s1-List1->DE < List1->num && s1->DocNum < s2->DocNum) s1++;
    }    
...
DocList *MergeLists (DocList *List1, DocList *List2)
{
  ...
  if (!(NewList = MakeDocList(List1->num + List2->num)))
    { Xfree(List1); Xfree(List2); return(NULL); }
