...
void average_space_enlarge()
{
  ...
  if(libsize>=avglibsize){
    ...
    if(!avglib)
      error("mgticprune","out of memory in reallocating..","");
...
/* for each mark in 'origlist', if it can't be matched with one in 
   ...
   avglib[] position */
void greedy_add(marklistptr origlist, marklistptr *librarylist)
{
  ...
  for(i=0,step_i=origlist; step_i ;i++,step_i=step_i->next){
    ...
    if(notfound){ 
      ...
      libsize++;
      if(libsize>=avglibsize) average_space_enlarge();
    }
  }
}
...
void greedy_prune(marklistptr *library)
{
  int count;
  int i,j;
  marklistptr step;
...
void prune_on_size(marklistptr *list, int sizethres)
{
  int count,i;
  marktype d;
...
void prune_on_area(marklistptr *list, int areathres)
{
  int count,i;
  marktype d;
...
void averagedout_addagain(marklistptr origlist, marklistptr *librarylist)
{
...
void main(int argc, char *args[])
{
	...
	for(i=1;i<argc;i++){
	    if(!strcmp(args[i],"-v")) V=1;
	    else if(!strcmp(args[i],"-h")) usage();
	    else if(!strcmp(args[i],"-Q")) {num_passes=0;}
	    else if(!strcmp(args[i],"-s")) {i++;
	      if((i<argc)&&(isinteger(args[i]))) sizethres=atoi(args[i]);
	      else error(args[0],"follow the -s option with a numeric arg >=0","");
	    }
	    else if(!strcmp(args[i],"-a")) {i++;
	      if((i<argc)&&(isinteger(args[i]))) areathres=atoi(args[i]);
	      else error(args[0],"follow the -a option with a numeric arg >=0","");
	    }
	    else if(args[i][0]=='-') error(args[0],"unknown switch:",args[i]);
	    else if(!libraryname) libraryname=args[i];
	    else if(!prunename) prunename=args[i];
	...
	if(!prunename) {
	  ...
	  if(V) fprintf(stderr,"overwriting previous library file.\n");
	  overwrite=1;
	}
	...
	if(V) fprintf(stderr,"%d marks...",count);

	avglib=(marklistptr*)malloc(count*sizeof(marklistptr));
	if(!avglib) error(args[0],"out of memory","");
	...
	if(V) fprintf(stderr,"after threshold pruning, %d marks\n",count=marklist_length(origlist));

	  
	if(V) fprintf(stderr,"starting greedy library creation...\n");
	...
	for(pass=0; pass<num_passes; pass++){
	  ...
	  if(V) fprintf(stderr,"averaging...\n");
	  librarylist=average_library();
	  averagedout_addagain(origlist,&librarylist);
	  greedy_prune(&librarylist);
	  if(V) fprintf(stderr,"after pass %d greedy prune %d marks\n",1+pass,marklist_length(librarylist));
	}


	lib=fopen(tempn,"wb");
	if(lib==NULL) error(args[0],"trouble creating prune-library file.","");


	if(V) fprintf(stderr,"writing to file: %s...",prunename);
