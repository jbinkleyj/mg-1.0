...
int main(int argc, char **argv)
{
  ...
  for(i=1;i<argc;i++){
      ...
      else if(!strcmp(argv[i],"-e")) encode_flag=1;
      else if(!strcmp(argv[i],"-d")) decode_flag=1;
      else if(!strcmp(argv[i],"-k")) {maxk=atoi(argv[++i]);}
      else if(argv[i][0]=='-') error(argv[0],"unknown switch:",argv[i]);
      else if(!filename) filename=argv[i];
      else error(argv[0],"too many filenames specified.","");
  }

  if(encode_flag==decode_flag) error(argv[0],"please specify either encode XOR decode.","");

  if(!filename) error(argv[0],"please specify a filename.","");

  if ((encode_flag)&&(maxk >= MAXKVALUE)) {                       /* to fit into 32 bit integers */
  ...
  if((fp_in = fopen(filename,"rb"))==NULL)
    error(argv[0], "unable to open file:", filename);
  
  /* read file header to determine the type and dimensions */
  
  
  magic=magic_read(fp_in);
  if(magic!=MAGIC_FELICS){rewind(fp_in); magic=getmagicno_short_pop(fp_in);}

  if((decode_flag && (magic!=MAGIC_FELICS)) ||
     (encode_flag && ((magic!=MAGIC_P2) && (magic!=MAGIC_P5)))){
    fclose(fp_in);
    error(argv[0], encode_flag?"only PGM files can be used.":"only felics compressed files can be used.","");
  }
  ...
  if ((line = (unsigned int *) malloc(width * sizeof(int *))) == NULL) {
      fclose(fp_in);
      error(argv[0], "unable to allocate memory,", "");
  }
  ...
  if(decode_flag){
      if (maxk) warn(argv[0],"k value not required for decompression, ignored.","");
