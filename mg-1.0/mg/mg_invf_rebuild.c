 ...
 *
 *       @(#)mg_invf_rebuild.c	1.6 16 Mar 1994
 *
 **************************************************************************/
...
char *SCCS_Id_invf_rebuild = "@(#)mg_invf_rebuild.c	1.6 16 Mar 1994";
...
static void process_files(char *filename)
{
...
#ifdef SUNOS5 
  memset(ifh_out.params, 0, sizeof(ifh_out.params));
#else
  ...
  switch (outmode)
    {
      case 0 : break;
      case 1 : ifh_out.params[0] = k; break;
      case 2 : ifh_out.params[0] = max_nodes;
	       ifh_out.params[1] = mins; break;
    }
  ...
  for (i=0; i<ifh_in.no_of_words; i++)
    {
      ...
      fgetc(dict);
      suff = fgetc(dict);
      fread(dummy2, sizeof(uchar), suff, dict);
