...
int WXOR_match(marktype b1, marktype b2)
{
  ...
  for(r=minr;r<maxr;r++)
    for(c=minc;c<maxc;c++){
      if( (r>=0) && (c>=0) && (r<b1.h) && (c<b1.w) ) p1=pbm_getpixel(b1.bitmap,c,r);
      else p1=0; /* 0 = back ground colour */
      if( (r-offy>=0) && (c-offx>=0) && (r-offy<b2.h) && (c-offx<b2.w) ) p2=pbm_getpixel(b2.bitmap,c-offx,r-offy);
      else p2=0; /* background colour */
...
static void xormatchscratch(marktype *b1, marktype *b2, int minr, int maxr, int minc, int maxc, int offx, int offy)
{
  ...
  for(r=minr; r<maxr;r++){
    ...
    for(c=minc;c<0;c++){
      ...
      if((coo>=0)&&(roo>=0)&&(coo<w2)&&(roo<h2))p2=pbm_getpixel(bm2,coo,roo);
    ...
    for(c=0;c<maxc;c++){
      coo=c-offx;
      scratch[rmm][c-minc]=sc2[rmm][c-minc]=0;
      if((r>=0)&&(c<w1)&&(r<h1))p1=pbm_getpixel(bm1,c,r);
...
int CSIS_match(marktype *b1, marktype *b2)
{
	...
	for(i=1;i<8;i+=3)
	  if(((r+oc[i].y)>=minr)&&((c+oc[i].x)>=minc)&&((r+oc[i].y)<maxr)&&((c+oc[i].x)<maxc))
	    if(scratch[r+oc[i].y-minr][c+oc[i].x-minc]) sc2[r-minr][c-minc]++;
      }
